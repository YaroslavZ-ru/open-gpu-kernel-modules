...
// Class-specific details for MpsApi
const struct NVOC_CLASS_DEF __nvoc_class_def_MpsApi = 
{
    .classInfo.size =               sizeof(MpsApi),
    .classInfo.classId =            classId(MpsApi),
...
NV_STATUS __nvoc_ctor_MpsApi(MpsApi *pMpsApi, CALL_CONTEXT *pCallContext, struct RS_RES_ALLOC_PARAMS_INTERNAL *pParams) {    // inline
    NV_STATUS status = NV_OK;

    // Recurse to ancestor constructor(s).
    status = __nvoc_ctor_RmResource(&pMpsApi->__nvoc_base_RmResource, pCallContext, pParams);
    if (status != NV_OK) goto __nvoc_ctor_MpsApi_fail_RmResource;

    // Initialize data fields.
    __nvoc_init_dataField_MpsApi(pMpsApi);

    // Call the constructor for this class.
    status = __nvoc_mpsApiConstruct(pMpsApi, pCallContext, pParams);
...
NV_STATUS __nvoc_objCreate_MpsApi(MpsApi **__nvoc_ppThis, Dynamic *__nvoc_pParent, NvU32 __nvoc_createFlags, CALL_CONTEXT *pCallContext, struct RS_RES_ALLOC_PARAMS_INTERNAL *pParams)
{
    ...
    {
        __nvoc_pThis = portMemAllocNonPaged(sizeof(MpsApi));
        NV_CHECK_OR_RETURN(LEVEL_ERROR, __nvoc_pThis != NULL, NV_ERR_NO_MEMORY);
    }

    // Zero is the initial value for everything.
    portMemSet(__nvoc_pThis, 0, sizeof(MpsApi));
    ...
    __nvoc_init__MpsApi(__nvoc_pThis);
    __nvoc_status = __nvoc_ctor_MpsApi(__nvoc_pThis, pCallContext, pParams);
    if (__nvoc_status != NV_OK) goto __nvoc_objCreate_MpsApi_cleanup;
