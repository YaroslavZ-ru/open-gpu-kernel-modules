...
NV_STATUS __nvoc_ctor_MemoryMapper(MemoryMapper*, struct CALL_CONTEXT *pCallContext, struct RS_RES_ALLOC_PARAMS_INTERNAL *pParams);
...
// Class-specific details for MemoryMapper
...
NV_STATUS __nvoc_ctor_MemoryMapper(MemoryMapper *pMemoryMapper, struct CALL_CONTEXT *pCallContext, struct RS_RES_ALLOC_PARAMS_INTERNAL *pParams) {    // inline
    NV_STATUS status = NV_OK;

    // Recurse to ancestor constructor(s).
    status = __nvoc_ctor_GpuResource(&pMemoryMapper->__nvoc_base_GpuResource, pCallContext, pParams);
    if (status != NV_OK) goto __nvoc_ctor_MemoryMapper_fail_GpuResource;

    // Initialize data fields.
    __nvoc_init_dataField_MemoryMapper(pMemoryMapper);

    // Call the constructor for this class.
    status = __nvoc_memmapperConstruct(pMemoryMapper, pCallContext, pParams);
...
__nvoc_ctor_MemoryMapper_fail__init:
    __nvoc_dtor_GpuResource(&pMemoryMapper->__nvoc_base_GpuResource);
...
NV_STATUS __nvoc_objCreate_MemoryMapper(MemoryMapper **__nvoc_ppThis, Dynamic *__nvoc_pParent, NvU32 __nvoc_createFlags, struct CALL_CONTEXT *pCallContext, struct RS_RES_ALLOC_PARAMS_INTERNAL *pParams)
{
    NV_STATUS __nvoc_status;
    Object *__nvoc_pParentObj = NULL;
    MemoryMapper *__nvoc_pThis;
    ...
    {
        __nvoc_pThis = portMemAllocNonPaged(sizeof(MemoryMapper));
        NV_CHECK_OR_RETURN(LEVEL_ERROR, __nvoc_pThis != NULL, NV_ERR_NO_MEMORY);
    }

    // Zero is the initial value for everything.
    portMemSet(__nvoc_pThis, 0, sizeof(MemoryMapper));
    ...
    __nvoc_init__MemoryMapper(__nvoc_pThis);
    __nvoc_status = __nvoc_ctor_MemoryMapper(__nvoc_pThis, pCallContext, pParams);
    if (__nvoc_status != NV_OK) goto __nvoc_objCreate_MemoryMapper_cleanup;
...
NV_STATUS __nvoc_objCreateDynamic_MemoryMapper(MemoryMapper **__nvoc_ppThis, Dynamic *__nvoc_pParent, NvU32 __nvoc_createFlags, va_list __nvoc_args) {
    NV_STATUS __nvoc_status;
    struct CALL_CONTEXT *pCallContext = va_arg(__nvoc_args, struct CALL_CONTEXT *);
    struct RS_RES_ALLOC_PARAMS_INTERNAL *pParams = va_arg(__nvoc_args, struct RS_RES_ALLOC_PARAMS_INTERNAL *);

    __nvoc_status = __nvoc_objCreate_MemoryMapper(__nvoc_ppThis, __nvoc_pParent, __nvoc_createFlags, pCallContext, pParams);
