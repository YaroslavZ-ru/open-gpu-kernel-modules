...
// Class-specific details for OBJOS
const struct NVOC_CLASS_DEF __nvoc_class_def_OBJOS = 
{
    .classInfo.size =               sizeof(OBJOS),
    .classInfo.classId =            classId(OBJOS),
...
NV_STATUS __nvoc_objCreate_OBJOS(OBJOS **__nvoc_ppThis, Dynamic *__nvoc_pParent, NvU32 __nvoc_createFlags)
{
    ...
    {
        __nvoc_pThis = portMemAllocNonPaged(sizeof(OBJOS));
        NV_CHECK_OR_RETURN(LEVEL_ERROR, __nvoc_pThis != NULL, NV_ERR_NO_MEMORY);
    }

    // Zero is the initial value for everything.
    portMemSet(__nvoc_pThis, 0, sizeof(OBJOS));
    ...
    __nvoc_init__OBJOS(__nvoc_pThis);
    __nvoc_status = __nvoc_ctor_OBJOS(__nvoc_pThis);
    if (__nvoc_status != NV_OK) goto __nvoc_objCreate_OBJOS_cleanup;
