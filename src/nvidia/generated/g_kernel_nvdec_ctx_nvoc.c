...
// Class-specific details for NvdecContext
const struct NVOC_CLASS_DEF __nvoc_class_def_NvdecContext = 
{
    .classInfo.size =               sizeof(NvdecContext),
    .classInfo.classId =            classId(NvdecContext),
...
NV_STATUS __nvoc_ctor_NvdecContext(NvdecContext *pNvdecContext, RmHalspecOwner *pRmhalspecowner, struct CALL_CONTEXT *pCallContext, struct RS_RES_ALLOC_PARAMS_INTERNAL *pParams) {    // inline body
    NV_STATUS status = NV_OK;

    // Recurse to ancestor constructor(s).
    status = __nvoc_ctor_ChannelDescendant(&pNvdecContext->__nvoc_base_ChannelDescendant, pRmhalspecowner, pCallContext, pParams, nvdecGetEngineDescFromAllocParams);
    if (status != NV_OK) goto __nvoc_ctor_NvdecContext_fail_ChannelDescendant;

    // Initialize data fields.
    __nvoc_init_dataField_NvdecContext(pNvdecContext, pRmhalspecowner);

    // Call the constructor for this class.
    status = __nvoc_nvdecctxConstruct(pNvdecContext, pCallContext, pParams);
...
NV_STATUS __nvoc_objCreate_NvdecContext(NvdecContext **__nvoc_ppThis, Dynamic *__nvoc_pParent, NvU32 __nvoc_createFlags, struct CALL_CONTEXT *pCallContext, struct RS_RES_ALLOC_PARAMS_INTERNAL *pParams)
{
    NV_STATUS __nvoc_status;
    Object *__nvoc_pParentObj = NULL;
    NvdecContext *__nvoc_pThis;
    ...
    {
        __nvoc_pThis = portMemAllocNonPaged(sizeof(NvdecContext));
        NV_CHECK_OR_RETURN(LEVEL_ERROR, __nvoc_pThis != NULL, NV_ERR_NO_MEMORY);
    }

    // Zero is the initial value for everything.
    portMemSet(__nvoc_pThis, 0, sizeof(NvdecContext));
    ...
    NV_CHECK_TRUE_OR_GOTO(__nvoc_status, LEVEL_ERROR, __nvoc_pParent != NULL, NV_ERR_INVALID_ARGUMENT, __nvoc_objCreate_NvdecContext_cleanup);
    ...
    NV_CHECK_TRUE_OR_GOTO(__nvoc_status, LEVEL_ERROR, pRmhalspecowner != NULL, NV_ERR_INVALID_ARGUMENT, __nvoc_objCreate_NvdecContext_cleanup);

    // Initialize vtable, RTTI, etc., then call constructor.
    __nvoc_init__NvdecContext(__nvoc_pThis, pRmhalspecowner);
    __nvoc_status = __nvoc_ctor_NvdecContext(__nvoc_pThis, pRmhalspecowner, pCallContext, pParams);
    if (__nvoc_status != NV_OK) goto __nvoc_objCreate_NvdecContext_cleanup;
