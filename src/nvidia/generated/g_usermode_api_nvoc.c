...
// Class-specific details for UserModeApi
const struct NVOC_CLASS_DEF __nvoc_class_def_UserModeApi = 
{
    .classInfo.size =               sizeof(UserModeApi),
    .classInfo.classId =            classId(UserModeApi),
...
// Construct UserModeApi object.
NV_STATUS __nvoc_ctor_Memory(Memory *pMemory, CALL_CONTEXT *pCallContext, struct RS_RES_ALLOC_PARAMS_INTERNAL *pParams);    // inline
NV_STATUS __nvoc_ctor_UserModeApi(UserModeApi *pUserModeApi, CALL_CONTEXT *pCallContext, struct RS_RES_ALLOC_PARAMS_INTERNAL *pParams) {    // inline
    NV_STATUS status = NV_OK;

    // Recurse to ancestor constructor(s).
    status = __nvoc_ctor_Memory(&pUserModeApi->__nvoc_base_Memory, pCallContext, pParams);
    if (status != NV_OK) goto __nvoc_ctor_UserModeApi_fail_Memory;

    // Initialize data fields.
    __nvoc_init_dataField_UserModeApi(pUserModeApi);

    // Call the constructor for this class.
    status = __nvoc_usrmodeConstruct(pUserModeApi, pCallContext, pParams);
...
NV_STATUS __nvoc_objCreate_UserModeApi(UserModeApi **__nvoc_ppThis, Dynamic *__nvoc_pParent, NvU32 __nvoc_createFlags, CALL_CONTEXT *pCallContext, struct RS_RES_ALLOC_PARAMS_INTERNAL *pParams)
{
    NV_STATUS __nvoc_status;
    Object *__nvoc_pParentObj = NULL;
    UserModeApi *__nvoc_pThis;
    ...
    {
        __nvoc_pThis = portMemAllocNonPaged(sizeof(UserModeApi));
        NV_CHECK_OR_RETURN(LEVEL_ERROR, __nvoc_pThis != NULL, NV_ERR_NO_MEMORY);
    }

    // Zero is the initial value for everything.
    portMemSet(__nvoc_pThis, 0, sizeof(UserModeApi));
    ...
    __nvoc_init__UserModeApi(__nvoc_pThis);
    __nvoc_status = __nvoc_ctor_UserModeApi(__nvoc_pThis, pCallContext, pParams);
    if (__nvoc_status != NV_OK) goto __nvoc_objCreate_UserModeApi_cleanup;
