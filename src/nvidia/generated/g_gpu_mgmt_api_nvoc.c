...
// Class-specific details for GpuManagementApi
const struct NVOC_CLASS_DEF __nvoc_class_def_GpuManagementApi = 
{
    .classInfo.size =               sizeof(GpuManagementApi),
    .classInfo.classId =            classId(GpuManagementApi),
...
// Construct GpuManagementApi object.
...
NV_STATUS __nvoc_ctor_GpuManagementApi(GpuManagementApi *pGpuMgmt, struct CALL_CONTEXT *pCallContext, struct RS_RES_ALLOC_PARAMS_INTERNAL *pParams) {    // inline
    NV_STATUS status = NV_OK;

    // Recurse to ancestor constructor(s).
    status = __nvoc_ctor_RmResource(&pGpuMgmt->__nvoc_base_RmResource, pCallContext, pParams);
    if (status != NV_OK) goto __nvoc_ctor_GpuManagementApi_fail_RmResource;

    // Initialize data fields.
    __nvoc_init_dataField_GpuManagementApi(pGpuMgmt);

    // Call the constructor for this class.
    status = __nvoc_gpumgmtapiConstruct(pGpuMgmt, pCallContext, pParams);
...
NV_STATUS __nvoc_objCreate_GpuManagementApi(GpuManagementApi **__nvoc_ppThis, Dynamic *__nvoc_pParent, NvU32 __nvoc_createFlags, struct CALL_CONTEXT *pCallContext, struct RS_RES_ALLOC_PARAMS_INTERNAL *pParams)
{
    NV_STATUS __nvoc_status;
    Object *__nvoc_pParentObj = NULL;
    GpuManagementApi *__nvoc_pThis;
    ...
    {
        __nvoc_pThis = portMemAllocNonPaged(sizeof(GpuManagementApi));
        NV_CHECK_OR_RETURN(LEVEL_ERROR, __nvoc_pThis != NULL, NV_ERR_NO_MEMORY);
    }

    // Zero is the initial value for everything.
    portMemSet(__nvoc_pThis, 0, sizeof(GpuManagementApi));
    ...
    __nvoc_init__GpuManagementApi(__nvoc_pThis);
    __nvoc_status = __nvoc_ctor_GpuManagementApi(__nvoc_pThis, pCallContext, pParams);
    if (__nvoc_status != NV_OK) goto __nvoc_objCreate_GpuManagementApi_cleanup;
