...
// Class-specific details for TimerApi
const struct NVOC_CLASS_DEF __nvoc_class_def_TimerApi = 
{
    .classInfo.size =               sizeof(TimerApi),
    .classInfo.classId =            classId(TimerApi),
...
NV_STATUS __nvoc_ctor_TimerApi(TimerApi *pTimerApi, struct CALL_CONTEXT *pCallContext, struct RS_RES_ALLOC_PARAMS_INTERNAL *pParams) {    // inline
    NV_STATUS status = NV_OK;

    // Recurse to ancestor constructor(s).
    status = __nvoc_ctor_GpuResource(&pTimerApi->__nvoc_base_GpuResource, pCallContext, pParams);
    if (status != NV_OK) goto __nvoc_ctor_TimerApi_fail_GpuResource;
    status = __nvoc_ctor_Notifier(&pTimerApi->__nvoc_base_Notifier, pCallContext);
    if (status != NV_OK) goto __nvoc_ctor_TimerApi_fail_Notifier;

    // Initialize data fields.
    __nvoc_init_dataField_TimerApi(pTimerApi);

    // Call the constructor for this class.
    status = __nvoc_tmrapiConstruct(pTimerApi, pCallContext, pParams);
...
NV_STATUS __nvoc_objCreate_TimerApi(TimerApi **__nvoc_ppThis, Dynamic *__nvoc_pParent, NvU32 __nvoc_createFlags, struct CALL_CONTEXT *pCallContext, struct RS_RES_ALLOC_PARAMS_INTERNAL *pParams)
{
    NV_STATUS __nvoc_status;
    Object *__nvoc_pParentObj = NULL;
    TimerApi *__nvoc_pThis;
    ...
    {
        __nvoc_pThis = portMemAllocNonPaged(sizeof(TimerApi));
        NV_CHECK_OR_RETURN(LEVEL_ERROR, __nvoc_pThis != NULL, NV_ERR_NO_MEMORY);
    }

    // Zero is the initial value for everything.
    portMemSet(__nvoc_pThis, 0, sizeof(TimerApi));
    ...
    __nvoc_init__TimerApi(__nvoc_pThis);
    __nvoc_status = __nvoc_ctor_TimerApi(__nvoc_pThis, pCallContext, pParams);
    if (__nvoc_status != NV_OK) goto __nvoc_objCreate_TimerApi_cleanup;
