...
// Construct SyncpointMemory object.
NV_STATUS __nvoc_ctor_Memory(Memory *pMemory, CALL_CONTEXT *pCallContext, struct RS_RES_ALLOC_PARAMS_INTERNAL *pParams);    // inline
NV_STATUS __nvoc_ctor_SyncpointMemory(SyncpointMemory *pSyncpointMemory, CALL_CONTEXT *pCallContext, struct RS_RES_ALLOC_PARAMS_INTERNAL *pParams) {    // inline
    NV_STATUS status = NV_OK;

    // Recurse to ancestor constructor(s).
    status = __nvoc_ctor_Memory(&pSyncpointMemory->__nvoc_base_Memory, pCallContext, pParams);
    if (status != NV_OK) goto __nvoc_ctor_SyncpointMemory_fail_Memory;

    // Initialize data fields.
    __nvoc_init_dataField_SyncpointMemory(pSyncpointMemory);

    // Call the constructor for this class.
    status = __nvoc_syncpointConstruct(pSyncpointMemory, pCallContext, pParams);
...
NV_STATUS __nvoc_objCreate_SyncpointMemory(SyncpointMemory **__nvoc_ppThis, Dynamic *__nvoc_pParent, NvU32 __nvoc_createFlags, CALL_CONTEXT *pCallContext, struct RS_RES_ALLOC_PARAMS_INTERNAL *pParams)
{
    NV_STATUS __nvoc_status;
    Object *__nvoc_pParentObj = NULL;
    SyncpointMemory *__nvoc_pThis;
    ...
    {
        __nvoc_pThis = portMemAllocNonPaged(sizeof(SyncpointMemory));
        NV_CHECK_OR_RETURN(LEVEL_ERROR, __nvoc_pThis != NULL, NV_ERR_NO_MEMORY);
    }

    // Zero is the initial value for everything.
    portMemSet(__nvoc_pThis, 0, sizeof(SyncpointMemory));
    ...
    __nvoc_init__SyncpointMemory(__nvoc_pThis);
    __nvoc_status = __nvoc_ctor_SyncpointMemory(__nvoc_pThis, pCallContext, pParams);
    if (__nvoc_status != NV_OK) goto __nvoc_objCreate_SyncpointMemory_cleanup;
