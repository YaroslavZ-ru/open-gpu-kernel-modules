...
// Class-specific details for RgLineCallback
const struct NVOC_CLASS_DEF __nvoc_class_def_RgLineCallback = 
{
    .classInfo.size =               sizeof(RgLineCallback),
    .classInfo.classId =            classId(RgLineCallback),
...
// Construct RgLineCallback object.
...
NV_STATUS __nvoc_ctor_RgLineCallback(RgLineCallback *pRgLineCallback, struct CALL_CONTEXT *pCallContext, struct RS_RES_ALLOC_PARAMS_INTERNAL *pParams) {    // inline
    NV_STATUS status = NV_OK;

    // Recurse to ancestor constructor(s).
    status = __nvoc_ctor_GpuResource(&pRgLineCallback->__nvoc_base_GpuResource, pCallContext, pParams);
    if (status != NV_OK) goto __nvoc_ctor_RgLineCallback_fail_GpuResource;

    // Initialize data fields.
    __nvoc_init_dataField_RgLineCallback(pRgLineCallback);

    // Call the constructor for this class.
    status = __nvoc_rglcbConstruct(pRgLineCallback, pCallContext, pParams);
...
NV_STATUS __nvoc_objCreate_RgLineCallback(RgLineCallback **__nvoc_ppThis, Dynamic *__nvoc_pParent, NvU32 __nvoc_createFlags, struct CALL_CONTEXT *pCallContext, struct RS_RES_ALLOC_PARAMS_INTERNAL *pParams)
{
    NV_STATUS __nvoc_status;
    Object *__nvoc_pParentObj = NULL;
    RgLineCallback *__nvoc_pThis;
    ...
    {
        __nvoc_pThis = portMemAllocNonPaged(sizeof(RgLineCallback));
        NV_CHECK_OR_RETURN(LEVEL_ERROR, __nvoc_pThis != NULL, NV_ERR_NO_MEMORY);
    }

    // Zero is the initial value for everything.
    portMemSet(__nvoc_pThis, 0, sizeof(RgLineCallback));
    ...
    __nvoc_init__RgLineCallback(__nvoc_pThis);
    __nvoc_status = __nvoc_ctor_RgLineCallback(__nvoc_pThis, pCallContext, pParams);
    if (__nvoc_status != NV_OK) goto __nvoc_objCreate_RgLineCallback_cleanup;
